% Reference: http://www.cs.toronto.edu/~tijmen/csc321/assignments/a2/
function a2(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  warning('error', 'Octave:broadcast');
  if exist('page_output_immediately'), page_output_immediately(1); endif
  more off;
...
function ret = loss(model, data, wd_coefficient)
  ...
  % weight decay loss. very straightforward: E = 1/2 * wd_coeffecient * parameters^2
  wd_loss = (sum(model.input_to_hid(:).^2) + sum(model.hid_to_class(:).^2))/2*wd_coefficient; 
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % The returned object <ret> is supposed to be exactly like parameter <model>, 
  % i.e. it has fields ret.input_to_hid and ret.hid_to_class, and those are of the same shape as they are in <model>.
  ...
  % Right now, it just returns a lot of zeros, which is obviously not the correct output. Your job is to change that.
