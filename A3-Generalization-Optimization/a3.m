%{
Announcement, added on November 9, 17:00 UTC. 
The most recent version of the gradient checker (see previous announcement) has a problem. 
When you run a3(1e6, 7, 10, 0, 0, false, 4) with it, 
after implementing the weight decay loss gradient but before implementing the classification loss gradient, 
it produces an error Theta element #1831 (part of hid_to_class), with value -2.227744e-03, 
has finite difference gradient -2.227695e+03 but analytic gradient -2.227744e+03. 
To overcome that problem, run a3(1e7, 7, 10, 0, 0, false, 4) instead (only the first parameter is different). 
Apparently, the weight decay has to be made even more dominant for this new gradient checker to forgive the fact that you don't have the classification loss gradient yet. 
I've modified the instructions below to use 1e7 instead of 1e6. 
...
Announcement, added on November 8, 01:37 UTC. 
Another small improvement has been made to the gradient checker: it now recognizes NaN's and infinity values as errors. 
%}
...
function a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  learning_rate_a = learning_rate / (1 + sqrt(optimization_iteration_i));
    ...
    theta = theta + momentum_speed * learning_rate_a;
...
% Gradient and Weight Regularization  
  ...
  W2grad = 1/M * (W2grad + wd_coefficient * model.hid_to_class);

  W1grad = zeros(size(model.input_to_hid));
  W1grad = W1grad + hidderv * data.inputs';
  W1grad = 1/M * (W1grad + wd_coefficient * model.input_to_hid);
